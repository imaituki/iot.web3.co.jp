...
<?php echo form_open($this->common_form_action_base . 'search/', array('method' => 'GET', 'id' => 'common_form', 'class' => 'well form-horizontal')); ?>
	...
	<div class="span12">
		<div class="control-group span5">
			<label class="control-label" for="">工種</label>
			<div class="controls">
                <?php echo form_nds_dropdown('cond_construction_type_id', $this->construction_type_id_search, $this->data, ' '); ?>
			</div>
		</div>
	</div>
	</div>
...
<?php foreach ($this->list as $value): ?>
	<tr>
		<td>
            <?php echo h($value['construction_category_name']); ?>
            <?php if (isset($value['construction_type_name'])): ?>&nbsp;:&nbsp;<?php endif; ?>
            <?php echo h($value['construction_type_name']); ?>
        </td>
		<td>
            <?php echo h($value['construction_detail_name']); ?>
            <?php if (isset($value['weight'])): ?>&nbsp;/&nbsp;<?php endif; ?>
            <?php echo h($value['weight']); ?>
            <?php if (isset($value['unit'])): ?>&nbsp;/&nbsp;<?php endif; ?>
            <?php echo h($value['unit']); ?>
            </td>

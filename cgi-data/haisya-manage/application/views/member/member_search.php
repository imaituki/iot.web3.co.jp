...
<?php echo form_open($this->common_form_action_base . 'search/', array('method' => 'GET', 'id' => 'common_form', 'class' => 'well form-horizontal')); ?>
	...
	<div class="span12">
		<div class="control-group span5">
			...
			<div class="controls">
				<?php echo form_nds_input('cond_keyword', $this->data, ' class="" placeholder="スペース区切りで複数キーワードを指定可能"'); ?>
				<span class="help-block"><?php echo $this->label_keyword_search_condition; ?>を検索します</span>
			</div>
		</div>
		<div class="control-group span5">
			<label class="control-label" for="">会員種別</label>
			<div class="controls">
				<?php echo form_nds_dropdown('cond_member_type', Member_type::get_dropdown_list(), $this->data, 'id="cond_account_type_label"'); ?>
			</div>
		</div>
	</div>
	</div>
...
<?php foreach ($this->list as $value): ?>
	<tr>
		...
		<?php endif; ?>
		<td>
			<?php echo h(Member_type::get_label($value['member_type'])); ?>
		</td>
		<td>
			<?php echo h($value['name']); ?>
		</td>
		<td>
			<?php echo h($value['email']); ?>
		</td>
		<td>

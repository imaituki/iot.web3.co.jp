...
<?php echo form_open($this->common_form_action_base . 'search/', array('method' => 'GET', 'id' => 'common_form', 'class' => 'well form-horizontal')); ?>
	...
	<div class="span12">
		<div class="control-group span5">
			<label class="control-label" for="">処理場</label>
			<div class="controls">
				<?php echo form_nds_input('cond_disposal_name', $this->data, 'id="cond_disposal_name_label"'); ?>
			</div>
		</div>
		<div class="control-group span5">
			<label class="control-label" for="">処理場(フリガナ)</label>
			<div class="controls">
				<?php echo form_nds_input('cond_disposal_furigana', $this->data, 'id="cond_disposal_furigana_label"'); ?>
			</div>
		</div>
	</div>
	</div>
...
<thead>
	<tr>
		<th>処理場
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/disposal_name/ASC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'disposal_name' && $cond_sort_order === 'ASC') ? 'color:red;' : ''; ?>">▲</a>
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/disposal_name/DESC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'disposal_name' && $cond_sort_order === 'DESC') ? 'color:red;' : ''; ?>">▼</a>
        </th>
		<th>処理場(フリガナ)
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/disposal_furigana/ASC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'disposal_furigana' && $cond_sort_order === 'ASC') ? 'color:red;' : ''; ?>">▲</a>
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/disposal_furigana/DESC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'disposal_furigana' && $cond_sort_order === 'DESC') ? 'color:red;' : ''; ?>">▼</a>
        </th>
		<th>登録日</th>
		<th>更新日</th>
		<th>&nbsp;</th>
...
<?php foreach ($this->list as $value): ?>
	<tr>
		<td><?php echo h($value['disposal_name']); ?>&nbsp;</td>
		<td><?php echo h($value['disposal_furigana']); ?>&nbsp;</td>

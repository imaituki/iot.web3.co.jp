...
<?php foreach ($this->list as $value): ?>
	<tr>
		...
		<?php if ($this->column_post_title_use): ?>
		<td>
			<?php if($value['id'] == Free_form_const::CONTACT): ?>
			<span class="label label-info">お問い合わせ用</span>
			<?php elseif($value['id'] == Free_form_const::MAIL_NEWS): ?>
			<span class="label label-info">メールニュース用</span>
			<?php endif; ?>
			<?php echo h($value['post_title']); ?>
		</td>
		<?php endif; ?>
		<td><?php echo h($value['to_email']); ?>　<?php echo h($value['to_email_2']); ?></td>
		<td>
			<?php if(($value['id'] == Free_form_const::CONTACT) or ($value['id'] == Free_form_const::MAIL_NEWS)): ?>
			<a href="<?php echo site_url("contact_apply/contact_apply_search/index/{$value['id']}"); ?>" class="btn btn-primary"><i class="icon-user icon-white"></i> 申込者一覧</a>
			<?php endif; ?>
		</td>
		<td>
			<a href="<?php echo site_url("{$this->page_path_edit}index/{$value['id']}"); ?>" class="btn btn-primary"><i class="icon-edit icon-white"></i>  編集</a>

			<?php if(($value['id'] != Free_form_const::CONTACT) && ($value['id'] != Free_form_const::MAIL_NEWS)): ?>

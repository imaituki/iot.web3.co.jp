...
<?php echo form_open($this->common_form_action_base . 'search/', array('method' => 'GET', 'id' => 'common_form', 'class' => 'well form-horizontal')); ?>
	...
	<div class="span12">
		<div class="control-group span5">
			<label class="control-label" for="">ナンバープレート</label>
			<div class="controls">
				<?php echo form_nds_input('cond_number_plate', $this->data, 'id="cond_number_plate_label"'); ?>
			</div>
		</div>
		<div class="control-group span5">
			<label class="control-label" for="">登録番号</label>
			<div class="controls">
				<?php echo form_nds_input('cond_registration_number', $this->data, 'id="cond_registration_number_label"'); ?>
			</div>
		</div>
	</div>
	</div>
...
<thead>
	<tr>
		<th>ナンバープレート
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/number_plate/ASC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'number_plate' && $cond_sort_order === 'ASC') ? 'color:red;' : ''; ?>">▲</a>
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/number_plate/DESC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'number_plate' && $cond_sort_order === 'DESC') ? 'color:red;' : ''; ?>">▼</a>
        </th>
		<th>登録番号
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/registration_number/ASC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'registration_number' && $cond_sort_order === 'ASC') ? 'color:red;' : ''; ?>">▲</a>
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/registration_number/DESC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'registration_number' && $cond_sort_order === 'DESC') ? 'color:red;' : ''; ?>">▼</a>
        </th>
		<th>登録日</th>
		<th>更新日</th>
		<th>&nbsp;</th>
...
<?php foreach ($this->list as $value): ?>
	<tr>
		...
		<td><?php echo h($value['registration_number']); ?>&nbsp;</td>

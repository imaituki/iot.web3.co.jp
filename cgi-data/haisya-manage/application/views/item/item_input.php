...
<tbody>
	...
	<tr>
		<th><?php echo $this->label_basic_category; ?>　<span class="label label-warning">必須</span></th>
		<td>

			<!-- 親子関係を持ったカテゴリーを出力する -->
			<?php if ( ! empty($this->basic_category_list)): ?>
			<?php foreach ($this->basic_category_list as $child_element): ?>
				<h5><?php echo h($child_element['parent_entity']->kubun_value); ?></h5>
				<p style="margin-left: 1em;">
				<?php foreach ($child_element['child_list'] as $entity): ?>
				<label class="checkbox inline" ><?php echo form_nds_multi_checkbox('basic_category_ids', $entity->id, $this->data); ?><?php echo h($entity->kubun_value); ?></label>　
				<?php endforeach; ?>
				</p>
			<?php endforeach; ?>
			<?php endif; ?>
	...
	<tr>
		...
		<td>
			<?php echo form_nds_dropdown('order_number', $this->order_number_list, $this->data, ' '); ?>

			<?php echo form_error('order_number'); ?>
			<span class="help-block">表示優先順位を選択してください。数字が大きいほうが優先されます。同じ数字の場合は登録が新しい順に表示されます。</span>
		</td>
	</tr>
	<?php endif; ?>
	<tr>
		<th>技術評価</th>
		<td>
			<?php if ( ! empty($this->technology_list)): ?>
			<?php foreach ($this->technology_list as $key => $value): ?>
			<label class="checkbox inline" ><?php echo form_nds_multi_checkbox('technology_ids', $key, $this->data); ?> <?php echo h($value); ?></label>
			<?php endforeach; ?>
			<?php endif; ?>

			<?php echo form_error('technology_ids'); ?>
		</td>
	</tr>
	<tr>
		<th>属性</th>
		<td>
			...
			<label class="checkbox inline" ><?php echo form_nds_multi_checkbox('other_attribute_ids', $key, $this->data); ?> <?php echo h($value); ?></label>
			<?php endforeach; ?>
			<?php endif; ?>

			<?php echo form_error('attribute_ids'); ?>
		</td>
	</tr>
	...
	<tr>
		<th>アクセス数</th>
		<td>
			<?php echo h($this->access_count); ?>
		</td>
	</tr>
	<?php endif; ?>
	<tr>
		<th>特別会員限定ダウンロードコンテンツ</th>
		<td>
			<?php if ( ! empty($this->download_file_category_list)): ?>
			<?php foreach ($this->download_file_category_list as $key => $value): ?>
			<label class="checkbox inline" ><?php echo form_nds_multi_checkbox('download_limited_kubun_ids', $key, $this->data); ?> <?php echo h($value); ?></label>
			<?php endforeach; ?>
			<?php endif; ?>
	
			<?php echo form_error('download_limited_kubun_ids'); ?>
			<span class="help-block">特別会員にのみファイルを公開する場合はチェックしてください。</span>
		</td>
	</tr>
</tbody>
</table>
...
<tbody>
	...
	<tr>
		<th class="span4"><?php echo $this->label_post_sub_title; ?>　<span class="label label-warning">必須</span></th>
		<td>
	...
	<tr>
		<th class="span4">
			...
			<span class="help-block">※最大500文字</span>
		</th>
		<td>
			<?php echo form_nds_textarea('post_content', $this->data, ' id="editor1" class="span6" rows="7"'); ?>
	...
	<tr>
		...
		<td>
			...
			<ul id="item_add_area">
			<?php for ($i=1; $i <= $this->item_select_max; $i++): ?>
				<?php $item_id = "related_item_id_{$i}"; ?>
				<?php $item_label = "related_item_label_{$i}"; ?>
				<li style="float: left;">
	
				<div class="input-append" style="margin-right: 20px;">
				<?php echo form_nds_dropdown($item_id, $this->item_list, $this->data, "id='{$item_id}' style='width:400px' "); ?>
				<a class="btn fancybox fancybox.iframe" href="<?php echo site_url("item/modal_item_search/index/{$item_id}/{$item_label}"); ?>"><i class="icon-search "></i></a>
				</div>
				<br />
				<?php echo form_error($item_id); ?>
				<?php echo error_msg($this->error_list, $item_id); ?>
				</li>
			<?php endfor; ?>
			</ul>
			...
			<?php echo form_error('related_item_id'); ?>

			<span class="help-block">製品のデータIDを選択してください。<i class="icon-search"></i>ボタンを押すとサブウィンドウで簡単に検索することができます。</span>
		</td>
	</tr>
</tbody>
</table>

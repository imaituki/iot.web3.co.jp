...
<?php echo form_open($this->common_form_action_base . 'search/', array('method' => 'GET', 'id' => 'common_form', 'class' => 'well form-horizontal')); ?>
	...
	<div class="form-actions" style="margin:0px auto;">
		...
		<button class="btn btn-danger" onclick="parent.$.fancybox.close();">閉じる</button>
	</div>
<?php echo form_close(); ?>
...
<?php foreach ($this->list as $value): ?>
	<tr>
		...
		<?php endif; ?>
		<?php if ($this->column_post_title_use): ?>
		<td><?php echo h($value['post_title']." / ".$value['post_title']); ?></td>
		<?php endif; ?>
		<td>
			<a href="#" 
			   onclick="
			       $('#<?php echo $parent_window_input_form_id; ?>',parent.document).val('<?php echo h($value['id']); ?>');
			       $('#<?php echo $parent_window_label_id; ?>',parent.document).text('<?php echo h($value['post_title']); ?>');
			       parent.$.fancybox.close();
			       return false;" 
			   class="btn btn-primary"> 選択</a>
		</td>
	</tr>
<?php endforeach; ?>
</tbody>

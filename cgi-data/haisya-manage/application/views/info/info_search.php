...
<?php foreach ($this->list as $value): ?>
	<tr>
		...
		<?php endif; ?>
		<?php if ($this->column_post_title_use): ?>
		<td><?php echo h($value['post_title']); ?>　<?php echo h($value['post_sub_title']); ?></td>
		<td>
			<?php if (is_not_blank($value['free_form_id'])): ?>
			<a href="<?php echo site_url("apply/apply_search/index/{$value['id']}"); ?>" class="btn btn-primary"><i class="icon-user icon-white"></i> 申込者一覧</a>
			<?php endif; ?>
		</td>
		<?php endif; ?>
		<td>

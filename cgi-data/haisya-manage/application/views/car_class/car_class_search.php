...
<?php echo form_open($this->common_form_action_base . 'search/', array('method' => 'GET', 'id' => 'common_form', 'class' => 'well form-horizontal')); ?>
	...
	<div class="span12">
		<div class="control-group span5">
			<label class="control-label" for="">車輌扱い名</label>
			<div class="controls">
				<?php echo form_nds_input('cond_car_class_name', $this->data, 'id="cond_car_class_name_label"'); ?>
			</div>
		</div>
	</div>
	</div>
...
<thead>
	<tr>
		<th>車輌扱い名
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/car_class_name/ASC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'car_class_name' && $cond_sort_order === 'ASC') ? 'color:red;' : ''; ?>">▲</a>
			<a href="<?php echo site_url()."/{$this->common_form_action_base}sort/car_class_name/DESC/?{$this->get_query}"; ?>" 
				style="<?php echo ($cond_sort_key === 'car_class_name' && $cond_sort_order === 'DESC') ? 'color:red;' : ''; ?>">▼</a>
        </th>
		<th>登録日</th>
		<th>更新日</th>
		<th>&nbsp;</th>
...
<?php foreach ($this->list as $value): ?>
	<tr>
		<td><?php echo h($value['car_class_name']); ?></td>

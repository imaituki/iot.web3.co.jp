...
<script src="<?php echo base_url('js/ckeditor/config.js'); ?>"></script>
...
$(document).ready(function(){
	CKEDITOR.replace( 'editor1',
			{
				toolbar : 'Full',
				contentsCss: '<?php echo base_url(); ?>css/front/style.css',
		...
		<td>
			<?php if ($this->page_type == Page_type::REGISTER): ?>
				<?php if ($this->basic_category_multi_select): ?>
					...
					<?php endforeach; ?>
				<?php else: ?>
					<?php echo form_nds_dropdown('basic_category_ids', $this->basic_category_list, $this->data, 'class=""'); ?>
				<?php endif; ?>
				<?php echo form_error('basic_category_ids'); ?>
			<?php else: ?>
				<?php echo h($this->target_kubun_label); ?>&nbsp;
			<?php endif; ?>
		</td>
	</tr>
	<?php endif; ?>

 ...
 *
 */
class Attendance_register extends Register_Controller
{
	/** フォームで使用するパラメータ名 */
	...
	private function _relation_check()
	{
		...
		if ($this->T_attendance->is_attendance_exists_date($this->data[self::F_STAFF_ID], substr($this->data[self::F_DATETIME], 0, 10)))
		{
			$ret = FALSE;
    ...
    {
        $this->data["datetime"] = (!isset($this->data["datetime"]) || $this->data["datetime"] == "") ? date("Y-m-d H:i") : $this->data["datetime"];
    }

 ...
 *
 */
class User_register extends Register_Controller
{
	/** フォームで使用するパラメータ名 */
	...
	{
		$this->form_validation->set_rules(self::F_USER_CODE, 'ユーザーコード', 'trim|required|alpha_dash|max_length[25]|min_length[3]');
		$this->form_validation->set_rules(self::F_USER_NAME, 'ユーザー名', 'trim|required|max_length[40]');
	...
	private function _relation_check()
	{
		...
		if ($this->M_user->is_user_exists($this->data[self::F_USER_CODE]))
		{
			$ret = FALSE;
			$this->error_list['user_code_dupricate'] = '入力されたユーザーコードは既に存在しているため使用できません。';
		}
	...
	{
		$entity = new M_user();
		$entity->user_code = $session_var[self::F_USER_CODE];

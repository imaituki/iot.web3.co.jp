 ...
 *
 */
class User_search extends Search_Controller 
{
	/** フォームで使用するパラメータ名 */
	const F_COND_USER_CODE='cond_user_code';
	const F_COND_USER_NAME='cond_user_name';
	const F_COND_ACCOUNT_TYPE='cond_account_type';
	...
	public function __construct()
	{
		...
		//独自のソート順
		$this->first_sort_key = 'user_furigana';
	...
	function delete($id = '')
	{
		...
		$user_entity = $this->delete_model->find($id);

		if ($user_entity->user_code == System_const::NDS_ROOT_USER)
		{
			$this->error_list['delete'] = 'このユーザは削除できません';
	...
	{
		//検索に必要な情報を作成
		$params = $this->_get_user_condition_params();
	 ...
	 */
	private function _get_user_condition_params()
	{
		$params = array();
		$params['user_code'] = $this->data[self::F_COND_USER_CODE];
		$params['keyword'] =  $this->data[self::F_COND_USER_NAME];
		$params['account_type'] = $this->data[self::F_COND_ACCOUNT_TYPE];

		if (! $this->login_user->is_nds_root()) 
		{
			//検索結果に含めないユーザーコード
			$params['ignore_user_codes'] = array(System_const::NDS_ROOT_USER);
		}

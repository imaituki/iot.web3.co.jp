 ...
 *
 */
class Googlemap_update extends Register_Controller
{
	/** CSVのインデックス用定数 */
...
//MH
	...
	function index()
	{
		$shop_list = $this->M_shop->find_all();
		...
		{
			if (is_blank($shop->place))
			{
				continue;
			}

			if (is_blank($shop->latitude)
			or is_blank($shop->longitude))
			{
				//echo "try";
				
				$map_info = $this->_get_map($shop->place);
				
				if (isset($map_info['latitude']) && isset($map_info['longitude']))
				{
					$shop->latitude = (float)$map_info['latitude'];
					$shop->longitude = (float)$map_info['longitude'];

					$this->M_shop->update("system", $shop);
				}
			}

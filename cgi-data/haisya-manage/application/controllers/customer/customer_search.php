 ...
 *
 */
class Customer_search extends Search_Controller 
{
	/** フォームで使用するパラメータ名 */
	const F_COND_COMPANY_NAME = 'cond_company_name';
	const F_COND_COMPANY_FURIGANA = 'cond_company_furigana';
	...
	public function __construct()
	{
		...
		$this->delete_model = $this->M_customer;
		$this->_page_setting();
		...
		//独自のソート順
		$this->first_sort_key = 'company_furigana';
	...
	{
		//検索に必要な情報を作成
		$params = $this->_get_customer_condition_params();
	 ...
	 */
	private function _get_customer_condition_params()
	{
		$params = array();
		$params['company_name'] =  $this->data[self::F_COND_COMPANY_NAME];
		$params['company_furigana'] =  $this->data[self::F_COND_COMPANY_FURIGANA];

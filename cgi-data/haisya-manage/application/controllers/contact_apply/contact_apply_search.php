 ...
 *
 */
class Contact_apply_search extends Post_search_Controller 
{
	const FREE_FORM_ID = 'free_form_id';
	var $free_form_id;
	var $apply_total;
	...
	public function __construct()
	{
		...
		$this->package_name = 'contact_apply';
		...
		{
			$this->free_form_id = $this->_get_page_session(self::FREE_FORM_ID);

			if ( ! $this->free_form_id)
			{
				show_error("フォームが指定されていません。");
			}
		}
	 ...
	 * 初期表示を行う。
	 */
	public function index($free_form_id)
	{
		$this->_unset_page_session();

		if ( ! $this->T_free_form->find($free_form_id))
		{
			show_error("フォームが選択されていません。");
		}

		$this->_save_page_session(self::FREE_FORM_ID, $free_form_id);
		$this->free_form_id = $free_form_id;
	...
	{
		$params = array();
		$params['relation_data_type'] = Relation_data_type::COMMON;
		$params['relation_data_id'] = Relation_data_id::DEFAULT_ID;

		$params['free_form_id'] = $this->free_form_id;
	...
	protected function _convert_for_csv($list)
	{
		...
		$free_form = $this->T_free_form->find($this->free_form_id);

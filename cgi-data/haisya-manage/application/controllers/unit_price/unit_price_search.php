 ...
 *
 */
class Unit_price_search extends Search_Controller 
{
	/** フォームで使用するパラメータ名 */
	const F_COND_CONSTRUCTION_TYPE_ID = 'cond_construction_type_id';

    /** 種別ID */
    //var $unit_price_id_list;
	...
	public function __construct()
	{
		...
		$this->delete_model = $this->M_unit_price;
	...
	{
		//検索に必要な情報を作成
		$params = $this->_get_unit_price_condition_params();
	 ...
	 */
	private function _get_unit_price_condition_params()
	{
		$params = array();
		$params['construction_type_id'] =  $this->data[self::F_COND_CONSTRUCTION_TYPE_ID];
//		$params['construction_detail_id'] =  $this->data[self::F_COND_CONSTRUCTION_DETAIL_ID];
//		$params['disposal_id'] =  $this->data[self::F_COND_DISPOSAL_ID];
//		$params['car_class_id'] =  $this->data[self::F_COND_CAR_CLASS_ID];
    ...
    {
        $this->construction_type_id_search = $this->M_construction_type->getConstructionTypeIdList();
    }

    public function get_unit_price_data()
    {
        $construction_type_id   = $this->input->get("construction_type_id");
        $construction_detail_id = $this->input->get("construction_detail_id");
        $disposal_id            = $this->input->get("disposal_id");
        $car_class_id           = $this->input->get("car_class_id");

        $entity = $this->M_unit_price->getUnitPrice( $construction_type_id, $construction_detail_id, $disposal_id, $car_class_id );
        if( $entity && count( $entity ) == 1 ){
            $entity_array = (array) $entity[0];

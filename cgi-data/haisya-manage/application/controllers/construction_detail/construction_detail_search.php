 ...
 *
 */
class Construction_detail_search extends Search_Controller 
{
	/** フォームで使用するパラメータ名 */
	const F_COND_CONSTRUCTION_DETAIL_CODE = 'cond_construction_detail_code';
	const F_COND_CLASSID                    = 'class_id';
	const F_COND_CONSTRUCTION_DETAIL_NAME = 'cond_construction_detail_name';
	...
	public function __construct()
	{
		...
		$this->delete_model = $this->M_construction_detail;
		$this->_page_setting();
		...
		$this->valid_sort_key = array(
			'construction_detail_code',
			'construction_detail_name',
			'weight',
			'unit'
		);

		//独自のソート順
		$this->first_sort_key = 'construction_detail_code';
	...
	function delete($id = '')
	{
		...
		$this->db->trans_start();
		//$this->M_construction_detail->delete($id);
	...
	{
		//検索に必要な情報を作成
		$params = $this->_get_construction_detail_condition_params();
	 ...
	 */
	private function _get_construction_detail_condition_params()
	{
		$params = array();
		$params['construction_detail_code'] =  $this->data[self::F_COND_CONSTRUCTION_DETAIL_CODE];
		$params['construction_detail_name'] =  $this->data[self::F_COND_CONSTRUCTION_DETAIL_NAME];

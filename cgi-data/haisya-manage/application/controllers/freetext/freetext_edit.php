 ...
 *
 */
class Freetext_edit extends Post_register_Controller 
{
	/* パラメータの追加があればここに記述 */

	var $target_kubun_label;
	...
	public function __construct()
	{
		...
		or  ! $this->_is_method_match('show_by_code'))
		{
			$post_id = $this->_get_page_session(self::POST_ID);

			$this->_init_label($post_id);
		}
	...
	/**
	 * 区分コードに一致するレコードを1件だけ取得して画面を表示する。
	 * 
	 * @param unknown_type $kubun_code
	 */
	public function show_by_code($kubun_code)
	{
		if ( ! is_num($kubun_code))
		{
		...
		{
			show_error("固定ページが作成されていません。");
		}

		$this->index($v_relation_entity->parent_relation_data_id);
	}
	 ...
	 * @param unknown_type $post_id
	 */
	private function _init_label($post_id)
	{
		...
		$this->target_kubun_label = $this->M_kubun->get_joined_label(
		                                                $this->target_data_type, 
		                                                Kubun_type::CATEGORY,
		                                                $category_ids
		                                            );

 ...
 *
 */
class Emergency_search extends Post_search_Controller 
{
	/*
	 * コンストラクタ
	 */
	public function __construct()
	{
		...
		$this->delete_model = $this->T_emergency;
		$this->_page_setting();
	...
	public function search($page = 1)
	{
		...
		//1件のみなので存在すれば変更画面にリダイレクト
		if ( ! empty($ret))
		{
			redirect(site_url("{$this->page_path_edit}index/{$ret[0]->id}"), 'location', 301);
			return ;
		}

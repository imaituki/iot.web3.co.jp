 ...
 *
 */
class Reserve_detail_register extends Register_Controller
{
	/** フォームで使用するパラメータ名 */
	...
	public function index($id = null)
	{
        ...
        $this->detail_number = $this->_getDetailNumber();
	...
	function back()
	{
        ...
        $this->_loadReserveData($session_var["reserve_id"]);
		$this->_do_back();
        $this->detail_number = $this->data["detail_number"];
		$this->_load_tpl($this->_get_view_name(View_type::INPUT), $this->data);
	}
	...
	private function _relation_check()
	{
        ...
        if( $reserve_time_start >= $reserve_time_end ){
            $ret = FALSE;
			$this->error_list['reserve_time_end'] = '終了時刻は開始時刻よりあとの時刻を指定して下さい';
        }
         */
        //$reserve_code = $this->data[self::F_RESERVE_ID];
		//if ($this->T_reserve->is_reserve_code_exists($reserve_code))
		//{
		//	$ret = FALSE;
		//	$this->error_list['reserve_code_duplicate'] = '入力された工事コードは既に登録されています。';
		//}

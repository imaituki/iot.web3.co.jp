 ...
 *
 */
class Item_search extends Post_search_Controller 
{
	const F_COND_OTHER_ATTRIBUTE_IDS = 'cond_other_attribute_ids';
	...
	public function __construct()
	{
		...
		/*
		 * 親データとなるパッケージなので保持しているセッション上の関連データを削除する。
		 * ※サブウィンドウでの検索の場合は除く
		 */

		if ( ! $this->_is_method_match('modal_search_json'))
		{
			$this->application_session_data->remove_stored_data($this->target_data_type);	
		}
	}
	 ...
	 * 
	 */
	public function modal_search_json()
	{
		$ret = $this->_do_search(1, $this->max_list);

		$converted_list = array();
		$converted_list = $this->_convert_list($ret);
	...
	/**
	 * (non-PHPdoc)
	 * @see Search_Controller::_init_relation_data()
	 */
	...
	protected function _get_condition_params()
	{
		...
		$column_kubun_id_condition_list = array();
		$column_kubun_id_condition_list[] = array(
			'kubun_type' => Kubun_type::OTHER_ATTRIBUTE,
			'kubun_ids' => create_array_param($this->data[self::F_COND_OTHER_ATTRIBUTE_IDS])
		);

		$params['column_kubun_id_condition_list'] = $column_kubun_id_condition_list;

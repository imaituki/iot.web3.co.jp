 ...
 *
 */
class Modal_item_search extends Post_search_Controller 
{
	//iframeで呼び出す親画面のフォーム部品のID属性値
	const PARENT_WINDOW_INPUT_FORM_ID = 'parent_window_input_form_id';
	const PARENT_WINDOW_LABEL_ID = 'parent_window_label_id';
	...
	public function __construct()
	{
		...
		$this->delete_model = $this->M_item;

		$this->layout[self::LAYOUT_LAYOUT] = $this->layout_dir."/layout_modal";
		...
		if ( ! $this->_is_method_match('index'))
		{
			$this->data[self::PARENT_WINDOW_INPUT_FORM_ID] = $this->_get_page_session(self::PARENT_WINDOW_INPUT_FORM_ID);
			$this->data[self::PARENT_WINDOW_LABEL_ID] = $this->_get_page_session(self::PARENT_WINDOW_LABEL_ID);
		}
	}
	 ...
	 * 初期表示を行う
	 * 
	 * @param unknown_type $parent_window_input_form_id
	 */
	public function index($parent_window_input_form_id, $parent_window_label_id)
	{
		$this->_unset_page_session();

		$this->data[self::PARENT_WINDOW_INPUT_FORM_ID] = $parent_window_input_form_id;
		$this->data[self::PARENT_WINDOW_LABEL_ID] = $parent_window_label_id;

		$this->_save_page_session(self::PARENT_WINDOW_INPUT_FORM_ID, $parent_window_input_form_id);
		$this->_save_page_session(self::PARENT_WINDOW_LABEL_ID, $parent_window_label_id);

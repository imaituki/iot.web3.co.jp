 ...
 *
 */
class Login_result_search extends Post_search_Controller 
{
	const F_COND_MEMBER_ID = 'cond_member_id';
	...
	public function __construct()
	{
		...
		$this->target_data_type = Relation_data_type::LOGIN_RESULT;
		$this->package_name = 'login_result';
		...
		$this->current_main_menu = 'keyword_search_result';
		$this->main_model = $this->T_login_result;
		$this->sub_model = FALSE;
		$this->delete_model = $this->T_login_result;
		$this->_page_setting();
	...
	private function _convert_list($list)
	{
		...
		foreach ($arraylist as $tmp)
		{
			...
			$tmp['member_id_label'] = $this->M_member->find_and_get_column($tmp['member_id'], 'email');
	...
	protected function _init_relation_data()
	{
		...
		$this->member_list = $this->M_member->create_dropdown_list_with_blank('--', 'id', 'name');
	}
	...
	protected function _get_condition_params()
	{
		...
		$params['member_id'] = $this->data[self::F_COND_MEMBER_ID];

 ...
 *
 */
class Construction_type_search extends Search_Controller 
{
	/** フォームで使用するパラメータ名 */
	const F_COND_CONSTRUCTION_CATEGORY_CODE = 'cond_construction_type_code';
//	const F_COND_CLASSID                    = 'construction_category_id';
	const F_COND_CONSTRUCTION_CATEGORY_NAME = 'cond_construction_type_name';
	...
	public function __construct()
	{
		...
		$this->delete_model = $this->M_construction_type;
		$this->class_model = $this->M_construction_category;
		$this->_page_setting();
		...
		//独自のソート順
		$this->first_sort_key = 'construction_type_code';
	...
	function delete($id = '')
	{
		...
		$this->db->trans_start();
		//$this->M_construction_type->delete($id);
	...
	{
		//検索に必要な情報を作成
		$params = $this->_get_construction_type_condition_params();
	 ...
	 */
	private function _get_construction_type_condition_params()
	{
		$params = array();
		$params['construction_type_code'] =  $this->data[self::F_COND_CONSTRUCTION_CATEGORY_CODE];
		$params['construction_type_name'] =  $this->data[self::F_COND_CONSTRUCTION_CATEGORY_NAME];

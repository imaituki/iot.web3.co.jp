 ...
 *
 */
class Member_edit extends Post_register_Controller 
{
	...
	private function _input_check()
	{
		...
		$this->form_validation->set_rules(self::F_PHONE_NUMBER, '電話番号', "{$required_str}callback_check_phone_number|max_length[200]");
		$this->form_validation->set_rules(self::F_FAX_NUMBER, 'FAX番号', "callback_check_phone_number|max_length[200]");
	...
	protected function _convert_load_data()
	{
		...
		$this->data[self::F_PASSWORD] = '';	//パスワードは画面には出さない
	}
	...
	protected function _set_main_table_column_for_update($session_var, $post_id)
	{
		...
		$main_entity->member_type = $session_var[self::F_MEMBER_TYPE];

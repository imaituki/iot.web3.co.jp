 ...
 *
 */
class Disposal_edit extends Register_Controller 
{
	const DISPOSAL_ID = 'disposal_id';
	...
	public function __construct()
	{
		...
		{
			$disposal_id = $this->_get_page_session(self::DISPOSAL_ID);
			$this->_init_label($disposal_id);
		}
	...
	public function index($id)
	{
		...
		$this->_save_page_session(self::DISPOSAL_ID, $id);
	 ...
	 */
	private function _init_label($disposal_id)
	{
		if ($disposal_id !== FALSE)
		{
			$disposal_entity = $this->M_disposal->find($disposal_id);
		}
	}
	...
	private function _relation_check()
	{
		...
		if ($this->M_disposal->is_disposal_exists_edit($this->data[self::F_DISPOSAL_NAME]
                                                      ,$this->_get_page_session(self::DISPOSAL_ID)))
		{
			$ret = FALSE;
	...
	{
		//セッションから情報を取得
		$id = $this->_get_page_session(self::DISPOSAL_ID);
	...
	{
		// 顧客情報を取得する
		$entity = $this->M_disposal->find($id);
	 ...
	 */
	private function _update_table($session_var, $disposal_id)
	{
		//最新状態のデータを取得
		$entity = $this->M_disposal->find($disposal_id);

 ...
 *
 */
class Car_class_edit extends Register_Controller 
{
	const CAR_CLASS_ID = 'car_class_id';

	/** フォームで使用するパラメータ名 */
	const F_CAR_CLASS_NAME = 'car_class_name';
	...
	public function __construct()
	{
		...
		$this->main_model = $this->M_car_class;
		$this->_page_setting();
		...
		{
			$car_class_id = $this->_get_page_session(self::CAR_CLASS_ID);
			$this->_init_label($car_class_id);
		}
	...
	public function index($id)
	{
		...
		$this->_save_page_session(self::CAR_CLASS_ID, $id);
	 ...
	 */
	private function _init_label($car_class_id)
	{
		if ($car_class_id !== FALSE)
		{
			$car_entity = $this->M_car_class->find($car_class_id);
		}
	}
	...
	{
		$this->form_validation->set_rules(self::F_CAR_CLASS_NAME, '車輌扱い名', 'trim|required|max_length[15]');
	...
	private function _relation_check()
	{
		...
		if ($this->M_car_class->is_car_class_name_exists_edit($this->data[self::F_CAR_CLASS_NAME]
                                                             ,$this->_get_page_session(self::CAR_CLASS_ID)))
		{
			$ret = FALSE;
			$this->error_list['car_class_name_duplicate'] = '入力された車輌扱い名は既に登録されています。';
		}
	...
	{
		//セッションから情報を取得
		$id = $this->_get_page_session(self::CAR_CLASS_ID);
	...
	{
		// 顧客情報を取得する
		$entity = $this->M_car_class->find($id);
	 ...
	 */
	private function _update_table($session_var, $car_class_id)
	{
		//最新状態のデータを取得
		$entity = $this->M_car_class->find($car_class_id);
		...
		//ユーザ入力値をセットしてUPDATE
		$entity->car_class_name = $session_var[self::F_CAR_CLASS_NAME];

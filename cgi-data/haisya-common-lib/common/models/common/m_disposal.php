 ...
 *
 */
class M_disposal extends Base_model 
{
	/** 処理場 */
	var $disposal_name;

	/** 処理場(フリガナ) */
	var $disposal_furigana;
	...
	/**
	 * 同じ処理場のレコードが存在するかどうかを取得する。（登録）
	 * 削除フラグは考慮しない
	 * 
	 * @param unknown_type $disposal_name
	 * @return true:存在する、false:存在しない
	 */
	function is_disposal_exists($disposal_name)
	{
		/*
		 * 削除フラグを考慮しない点に注意する
		 */
		...
		$result = $this->db
					->from($this->_table)
					->where('disposal_name', $disposal_name)
	 ...
	 * @return true:存在する、false:存在しない
	 */
	function is_disposal_exists_edit($disposal_name, $id)
	{
		/*
		 * 削除フラグを考慮しない点に注意する
		 */
...
*/
	...
	function select_for_manage(Nds_pagination $nds_pagination, $params, $count_only = FALSE)
	{
		...
		//処理場
		if (isset($params['disposal_name'])  && is_not_blank($params['disposal_name']))
		{
			$sql .= '
				AND MAIN.disposal_name LIKE ?
			';

			$value_array[] = "%{$params['disposal_name']}%";
		}

		//処理場(フリガナ)
		if (isset($params['disposal_furigana'])  && is_not_blank($params['disposal_furigana']))
		{
			$sql .= '
				AND MAIN.disposal_furigana LIKE ?
			';

			$value_array[] = "%{$params['disposal_furigana']}%";
		}
...
*/
	 ...
	 * プルダウン用配列を返す
	 */
	function getDisposalIdList( $with_no_val = false )
	{
		$value_array = array();
